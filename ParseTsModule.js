...
if (isWorker) {self.tsBrowser = tsBrowser} else {window.tsBrowser = tsBrowser}
    ...
    const consumeAst = (node) => {
        if (ts.SyntaxKind[node.kind] === 'CallExpression' &&
            ...
            const url = ts.SyntaxKind[arg.kind] !== 'StringLiteral' ? null :
                tsBrowser.addPathToUrl(arg.text, baseUrl);
...
tsBrowser.ParseTsModule_sideEffects = ({
    ...
    const filesource = fullUrl.replace(/^.*\//, '').replace("."+extension,'')
   
    const sourceFile = ts.createSourceFile(filesource+'.src.' + extension, tsCode, compilerOptions.target);
